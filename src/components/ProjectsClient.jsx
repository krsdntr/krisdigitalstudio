...
export default function ProjectsClient({ initialProjects }) {
    ...
    const handleLoadMore = async () => {
        ...
        try {
            const response = await fetch(`/api/projects?cursor=${nextCursor}`);
            ...
            {filteredAndSorted.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {filteredAndSorted.map((project) => {
                        const screenshotApiUrl = project.liveUrl
                            ? `https://s.wordpress.com/mshots/v1/${encodeURIComponent(project.liveUrl)}?w=1200`
                            : null;

                        const displayCover = project.cover || project.coverUrl || screenshotApiUrl;
